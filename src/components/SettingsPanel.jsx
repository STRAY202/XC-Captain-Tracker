...
import {
  ...
  ChevronLeft, ChevronRight, Sun, Moon, RotateCcw, ArrowUp, ArrowDown,
} from 'lucide-react';
      ...
      <div className="bg-white dark:bg-gray-900 w-full max-w-sm rounded-3xl p-6 shadow-2xl border border-gray-100 dark:border-gray-800">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <button
            type="submit"
            disabled={!code}
            className="w-full py-3.5 rounded-xl font-black text-sm bg-gray-900 dark:bg-white text-white dark:text-gray-900 disabled:opacity-40 hover:bg-gray-800 dark:hover:bg-gray-100 transition-colors min-h-[48px]"
          >
            Enter
          </button>
        </form>
...
function SlideEditor({ slidesKey, label }) {
  const { settings, updateOnboarding } = useApp();

  const [newIcon,  setNewIcon]  = useState('⭐');
  const [newTitle, setNewTitle] = useState('');
  const [newBody,  setNewBody]  = useState('');

  const rawSlides = settings.onboarding?.[slidesKey];
  const hasCustom = Array.isArray(rawSlides) && rawSlides.length > 0;
  const slides    = hasCustom ? rawSlides : [];

  const EMOJI_OPTIONS = ['⚡','✅','🟢','🏃','⚙️','📱','🏆','🌟','💪','🎯','📋','🗓️','📍','🤖','🎯','🏋️','📅'];

  const save = (newSlides) => {
    updateOnboarding({ [slidesKey]: newSlides.length > 0 ? newSlides : null });
  };
  ...
  return (
    <div className="space-y-3.5">
      <p className="text-xs text-gray-400 dark:text-gray-500 leading-relaxed">
        {hasCustom
          ? `Custom slides active (${slides.length} slide${slides.length !== 1 ? 's' : ''}). Reset to use built-in defaults.`
          : 'Using built-in defaults. Add slides below to override.'}
      </p>

      {hasCustom && (
        <div className="space-y-2">
          <p className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">{label} Slides</p>
          {slides.map((slide, idx) => (
            <div key={slide.id || idx} className="flex items-center gap-2 p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50">
              ...
              <div className="flex gap-1 flex-shrink-0">
                <button
                  ...
                  disabled={idx === 0}
                  className="w-6 h-6 rounded-lg bg-gray-100 dark:bg-gray-700 flex items-center justify-center disabled:opacity-30 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors active:scale-90"
                >
                  <ArrowUp size={10} className="text-gray-500" />
                </button>
                <button
                ...
                <button
                  ...
                  className="w-6 h-6 rounded-lg bg-red-50 dark:bg-red-950/30 flex items-center justify-center hover:bg-red-100 dark:hover:bg-red-900/50 transition-colors active:scale-90"
                >
          ...
          <button
            onClick={() => save([])}
            className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 text-xs font-bold text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors active:scale-[0.98]"
          >
            <RotateCcw size={12} /> Reset to built-in defaults
          </button>
        </div>
      )}
      ...
      <div className="space-y-2.5 bg-gray-50 dark:bg-gray-800/40 rounded-2xl p-4 border border-gray-100 dark:border-gray-800">
        <p className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Add Slide</p>
...
export default function SettingsPanel({ onClose, athleteMode = false, onReplayOnboarding }) {
  ...
  const weatherLat      = settings.onboarding?.weatherLat ?? 42.28;
  const weatherLon      = settings.onboarding?.weatherLon ?? -71.06;
  ...
  const handleResetOnboarding = () => {
    if (resetConfirm) {
      try { window.localStorage.removeItem('xc-ath-ob'); } catch {}
      try { window.localStorage.removeItem('xc-cap-ob'); } catch {}
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gray-50 dark:bg-gray-950 flex flex-col">
      ...
      {/* Header */}
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-800 flex-shrink-0 bg-white dark:bg-gray-900/80 backdrop-blur-xl pt-safe">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center flex-shrink-0 shadow-lg">
            <Zap size={16} className="text-white" fill="white" />
          </div>
          <div>
            <h2 className="font-black text-base text-gray-900 dark:text-white leading-tight">Settings</h2>
            <p className="text-[10px] text-gray-400 dark:text-gray-500 font-medium">App configuration</p>
          </div>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="overflow-y-auto flex-1 px-4 py-4 space-y-3 no-scrollbar">
        ...
        >
          <p className="text-xs text-gray-400 dark:text-gray-500 leading-relaxed">
            Coordinates used for the weather forecast shown on practice cards.
          </p>
          <div className="grid grid-cols-2 gap-3">
            <Field label="Latitude">
              <input
                ...
                step="0.01"
                value={weatherLat}
                onChange={e => updateOnboarding({ weatherLat: parseFloat(e.target.value) || 42.28 })}
                placeholder="42.28"
                className="field-input"
              />
            </Field>
            <Field label="Longitude">
              <input
                ...
                step="0.01"
                value={weatherLon}
                onChange={e => updateOnboarding({ weatherLon: parseFloat(e.target.value) || -71.06 })}
                placeholder="-71.06"
                className="field-input"
              />
            </Field>
        ...
        >
          {!isAdmin ? (
            <AdminLoginGate onClose={onClose} />
          ) : (
            <>
              ...
              <button
                onClick={() => { logoutAdmin(); }}
                className="text-xs text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 py-1.5 font-semibold"
              >
                Log out of admin
              </button>
            </>
                  ...
                  {CAPTAIN_COLORS.slice(0, 4).map(c => (
                    <button
                      ...
                      style={{
                        ...
                        outlineOffset: '1px',
                        boxShadow: newColor === c ? `0 0 3px ${c}44` : 'none',
                      }}
                    />
                  ))}
                </div>
        ...
        <Section
          ...
          defaultOpen={false}
        >
          <SlideEditor slidesKey="athleteSlides" label="Athlete" />
        </Section>

        {/* ── Captain Onboarding Editor ──────────────────────────────────── */}
        <Section
          ...
          defaultOpen={false}
        >
          <SlideEditor slidesKey="captainSlides" label="Captain" />
        </Section>

        {/* ── Onboarding Help Text ───────────────────────────────────────── */}
        <Section
          title="Onboarding Help Text"
          icon={BookOpen}
        ...
        >
          <p className="text-xs text-gray-400 dark:text-gray-500 leading-relaxed">
            Customize the welcome screen title and subtitle new users see. Leave empty to use built-in defaults.
          </p>
          ...
          {/* Legacy custom slides list */}
          {(settings.onboarding?.slides || []).length > 0 && (
            <div className="space-y-2">
              <p className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Legacy Custom Slides</p>
          ...
          <div className="space-y-2.5 bg-gray-50 dark:bg-gray-800/40 rounded-2xl p-4 border border-gray-100 dark:border-gray-800">
            <p className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Add Legacy Custom Slide</p>
            <div className="grid grid-cols-6 gap-1.5">
        ...
        >
          <p className="text-xs text-gray-400 dark:text-gray-500 leading-relaxed">
            Reset your onboarding so it shows again on next login. Useful for testing or if you want to re-read the intro.
          </p>
          <button
            ...
            className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-bold transition-all active:scale-[0.98] ${
              resetConfirm
                ...
                : 'bg-red-50 dark:bg-red-950/30 text-red-500 border border-red-200 dark:border-red-800 hover:bg-red-100 dark:hover:bg-red-900/40'
            }`}
          >
            <RotateCcw size={14} />
            {resetConfirm ? 'Tap again to confirm reset' : 'Reset My Onboarding'}
          </button>
          {resetConfirm && (
            <button
              onClick={() => setResetConfirm(false)}
              className="w-full text-xs text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 py-2 transition-colors"
            >
              Cancel
            </button>
          )}
